...
Build walls that bend the enemy route and chain traps. No ads, no IAP, offline
...
- 3 difficulties on every level, with par scores for bronze, silver and gold medals
...
NOTHING ELSE TO BUY
There are no in-app purchases, no premium currency and no energy timers. Every trap, enemy, level and difficulty is in the game from the first launch. There is no login and the app has no internet permission, so nothing about your play leaves your phone unless you send the save file yourself.
...
First release: 15 handmade levels in 3 regions, 3 difficulties with medals, 10 traps and 4 combos, 12 enemy types, a daily map, and a live route preview for every wall you place.
...
- Phone screenshots: `store/screenshots/` (6 PNG files, 1080x1920), captions in `store/captions.json`
